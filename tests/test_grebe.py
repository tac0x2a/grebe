...
def mocked_grebe():
    ...
    meta_store = MagicMock(name='meta_store')
    logger = MagicMock(name='logger')
    return (client, schema_store, meta_store, logger)
...
class TestCallBack():
    def test_insert_data_is_called(self, mocked_grebe, mocked_callback_args):
        (client, schema_store, meta_store, logger) = mocked_grebe
        schema_store.load_all_schemas.return_value = {}
        grebe = Grebe(client, schema_store, meta_store, 'exchange_name', 3, "UTC", logger)
